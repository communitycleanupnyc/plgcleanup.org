// Vendored from astro-delete-unused-images@1.0.3 — see ./index.js for why.
...
export async function getFilesRecursively(dir) {
  ...
  for (const c of content) {
    ...
    if (isDir) {
      output.push(...(await getFilesRecursively(join(dir, c))).map((f) => `${c}${sep}${f}`));
    } else {
      output.push(c);
    }
  }
...
export async function getFiles(dir, exts, filter) {
  return filterByExtensionArr(await getFilesRecursively(dir), exts).filter(filter);
}
