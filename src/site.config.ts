...
//   • the favicons       — generated by `npm run gen:logo` into public/, tinted
//                          from --accent/--bg in src/styles/tokens.css
// ─────────────────────────────────────────────────────────────────────────────
...
export const SITE = {
  ...
  /** Browser UI tint on mobile (the address bar behind a scrolled page). This
      site renders one theme in every OS setting, so it is a single value, not a
      light/dark pair — and it must BE `--bg` from tokens.css, or the bar and the
      page meet in a visible seam. `npm run audit` fails if the two disagree. */
  themeColor: "#00357a",
